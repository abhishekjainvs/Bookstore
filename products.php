...
<!-- Sidebar end=============================================== -->
	...
	<?php
		if(isset($_GET["cata"]))
		{
			...
			if($v=="0"||$v=="0000-00-00")
			{
				$val=mysqli_query($con,"select count(*) as `total` from `product` where `release date`>'$v' OR `popular`>'$v'");
				$data=mysqli_fetch_object($val);

				echo"<h3><small class='pull-right'>$data->total+ products are available </small></h3>	";
			}
			else
			{
				$val=mysqli_query($con,"select count(*) as `total` from `product` where `subgenre`='$v'");
				$data=mysqli_fetch_object($val);
			
				echo"<h3> $v <small class='pull-right'>$data->total+ products are available </small></h3>	";
		...
		<div class="control-group">
		  <label class="control-label alignL">Sort By </label>
			<select name="sort">
              ...
              <option value="3">Product Popularity</option>
			  <option value="4">New Release First</option>
              <option value="5">Price Lowest First</option>
            </select>
			<input type="submit" class="btn btn-primary" value="sort" name="sor" />
		</div>
	  </form>
...
<div class="tab-content">
	<div class="tab-pane" id="listView">
		...
		if(isset($_REQUEST["ser"]))
		{
			$sea=$_REQUEST["search"];
			$con=mysqli_connect("localhost","root","","ecommerce");
			$res=mysqli_query($con,"select * from `product` where `b_name`='$sea' OR `author`='$sea'");
			$result=mysqli_query($con,"select * from `product` where `b_name`='$sea' OR `author`='$sea'");
		...
		elseif(isset($_GET["cata"]))
		{
			...
			if($v=="0")
			{
					...
					$result=mysqli_query($con,"SELECT * FROM `product` ORDER BY `popular` desc");
					...
					while($val=mysqli_fetch_array($result))
		{
			...
			elseif($v=="0000-00-00")
			{
					$resu=mysqli_query($con,"SELECT * FROM `product` ORDER BY `release date` desc");
					
					while($val=mysqli_fetch_array($resu))
		{
			...
			$res=mysqli_query($con,"select * from `product` where `subgenre`='$v'");
		...
		elseif(isset($_REQUEST["go"]))
		{
			...
			$res=mysqli_query($con,"select * from `product` where `genre`='$gen'");
		...
		elseif(isset($_REQUEST["sor"]))
		{
			...
			if($so1==1)
			{
					$res=mysqli_query($con,"SELECT * FROM `product` ORDER BY `b_name`");
					...
					while($val=mysqli_fetch_array($res))
		{
			...
			if($so1==2)
			{
					$res=mysqli_query($con,"SELECT * FROM `product` ORDER BY `b_name` desc");
					...
					while($val=mysqli_fetch_array($res))
		{
			...
			if($so1==3)
			{
					$res=mysqli_query($con,"SELECT * FROM `product` where ORDER BY `popular` desc");
					...
					while($val=mysqli_fetch_array($res))
		{
			...
			if($so1==4)
			{
					$res=mysqli_query($con,"SELECT * FROM `product` ORDER BY `release date` desc");
					...
					while($val=mysqli_fetch_array($res))
		{
			...
			if($so1==5)
			{
					$res=mysqli_query($con,"SELECT * FROM `product` where ORDER BY `discounted_price`");
					...
					while($val=mysqli_fetch_array($res))
		{
			...
			{
				echo" <p style='text-align:center'> No items are selected.</p>";
			}
	...
	";
			...
			if($so1==3)
			{
					$res=mysqli_query($con,"SELECT * FROM `product` ORDER BY `popular` desc");
	...
	";
			...
			if($so1==5)
			{
					$res=mysqli_query($con,"SELECT * FROM `product` ORDER BY `discounted_price`");
